...
int ORBmatcher::SearchByProjection(std::shared_ptr<KeyFrame> pKF, const Eigen::Isometry3f &Tcw, const vector<std::shared_ptr<MapPoint>> &vpPoints, vector<std::shared_ptr<MapPoint>> &vpMatched, int th)
{
...
int ORBmatcher::SearchForTriangulation(std::shared_ptr<KeyFrame> pKF1, std::shared_ptr<KeyFrame> pKF2, const Eigen::Matrix3f &F12,
...
int ORBmatcher::Fuse(std::shared_ptr<KeyFrame> pKF, const Eigen::Isometry3f &Scw, const vector<std::shared_ptr<MapPoint>> &vpPoints, float th, vector<std::shared_ptr<MapPoint>> &vpReplacePoint)
{
    // Get Calibration Parameters for later projection
    const float &fx = pKF->fx;
    const float &fy = pKF->fy;
    ...
    // Decompose Scw
    Eigen::Matrix3f sRcw = Scw.rotation();
    const float scw = sqrt(sRcw.row(0).dot(sRcw.row(0)));
    Eigen::Matrix3f Rcw = sRcw/scw;
    Eigen::Vector3f tcw = Scw.translation()/scw;
    Eigen::Vector3f Ow = -Rcw.transpose()*tcw;
...
int ORBmatcher::SearchBySim3(std::shared_ptr<KeyFrame> pKF1, std::shared_ptr<KeyFrame> pKF2, vector<std::shared_ptr<MapPoint>> &vpMatches12,
                             const Eigen::Matrix3f &R12, const Eigen::Vector3f &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        const float invz = 1.0/p3Dc2.z();
        const float x = p3Dc2.x()*invz;
        const float y = p3Dc2.y()*invz;
    ...
    for(int i2=0; i2<N2; i2++)
    {
        ...
        Eigen::Vector3f p3Dc1 = R12*p3Dc2 + t12;

        // Depth must be positive
        if(p3Dc1.z()<0.0)
            continue;

        const float invz = 1.0/p3Dc1.z();
        const float x = p3Dc1.x()*invz;
        const float y = p3Dc1.y()*invz;
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th)
{
    ...
    const Eigen::Matrix3f Rcw = CurrentFrame.mTcw.rotation();
    const Eigen::Vector3f tcw = CurrentFrame.mTcw.translation();

    const Eigen::Vector3f twc = -Rcw.transpose()*tcw;

    const Eigen::Matrix3f Rlw = LastFrame.mTcw.rotation();
    const Eigen::Vector3f tlw = LastFrame.mTcw.translation();

    const Eigen::Vector3f tlc = Rlw*twc+tlw;

    const bool bForward = tlc.z()>CurrentFrame.mb;
    const bool bBackward = -tlc.z()>CurrentFrame.mb;
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                const float xc = x3Dc.x();
                const float yc = x3Dc.y();
                const float invzc = 1.0/x3Dc.z();
