...
namespace ORB_SLAM_CUSTOM {
        ...
        for(size_t i = 0, iend = mvpMapPointMatches.size(); i < iend; i++){
            ...
            if(pMP){
                if(!pMP -> isBad()){
                    ...
                    float x = (kp.pt.x-F.cx)/F.fx;
                    float y = (kp.pt.y-F.cy)/F.fy;
                    Eigen::Vector3f cv_br(x,y,1.f);
                    bearingVector_t br = cv_br.cast<double>();
	    ...
	    while(mnIterations<mRansacMaxIts || nCurrentIterations<nIterations)
	    {
	        ...
	        if(mnInliersi>=mRansacMinInliers)
	        {
	            ...
	            if(mnInliersi>mnBestInliers)
	            {
                  ...
                  cv::cv2eigen(Rcw, R);
                  cv::cv2eigen(tcw, t);
                  mBestTcw.block<3,3>(0,0) = R;
                  mBestTcw.block<3,1>(0,3) = t;
    ...
    void MLPnPsolver::CheckInliers(){
        ...
        for(int i=0; i<N; i++)
        {
            ...
            float u = fx*P3Dc.x/P3Dc.z + cx;
            float v = fy*P3Dc.y/P3Dc.z + cy;
            cv::Point2f uv(u,v);
    ...
    bool MLPnPsolver::Refine(){
        ...
        if(mnInliersi>mRansacMinInliers)
        {
            ...
            cv::cv2eigen(Rcw, R);
            cv::cv2eigen(tcw, t);
            mRefinedTcw.topLeftCorner<3,3>() = R;
            mRefinedTcw.block<3,1>(0,3) = t;
